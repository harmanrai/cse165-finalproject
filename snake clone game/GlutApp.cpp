...
void windowToScene(float& x, float& y) {
	...
	// added for a real x,y coordinate inside the projection area
	// normalized 0,0 -- 1,1
	x *= (singleton->winXmax - singleton->winXmin)+ singleton->winXmin;
	y *= (singleton->winYmax - singleton->winYmin)+singleton->winYmin;
}
...
void GlutApp::glutResizeCB(int w, int h) {
    ...
    if ((appXmax - appXmin) / w < (appYmax - appYmin) / h) {
		...
		singleton->winXmin = center - (center - appXmin)*scale;
		singleton->winXmax = center + (appXmax - center)*scale;
		singleton->winYmin = appYmin;
		singleton->winYmax = appYmax;
    }
    else {
        scale = ((appXmax - appXmin) / w) / ((appYmax - appYmin) / h);
        center = (appYmax + appYmin) / 2;
		singleton->winYmin = center - (center - appYmin)*scale;
		singleton->winYmax = center + (appYmax - center)*scale;
		singleton->winXmin = appXmin;
		singleton->winXmax = appXmax;
    }

    // Now we use glOrtho to set up our viewing frustum
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(singleton->winXmin, singleton->winXmax, singleton->winYmin, singleton->winYmax, -1, 1);
	//printf("%f %f %f %f\n", singleton->winXmin, singleton->winXmax, singleton->winYmin, singleton->winYmax);
}
